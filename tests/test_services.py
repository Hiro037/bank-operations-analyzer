...
class TestSimpleSearch(unittest.TestCase):
    ...
    def test_simple_search_found(self, mock_read_excel):
    ...
    @patch("pandas.read_excel")
    def test_simple_search_not_found(self, mock_read_excel):
        # Создаем мок DataFrame
        mock_data = pd.DataFrame({
            "Описание": ["Оплата интернета", "Оплата телефона"],
            "Категория": ["Интернет", "Телефон"]
        })
        mock_read_excel.return_value = mock_data
    ...
    @patch("pandas.read_excel")
    def test_simple_search_file_not_found(self, mock_read_excel):
    ...
    @patch("pandas.read_excel")
    def test_simple_search_general_exception(self, mock_read_excel):
