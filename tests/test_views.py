...
from src.views import (generate_home_page_response, get_card_summary,
...
def test_generate_home_page_response():
    # Мокируем функции
    with patch("src.views.get_greeting", return_value="Добрый день!"), \
         patch("src.views.get_transactions_for_month", return_value=create_test_transactions()), \
         patch("src.views.load_user_settings", return_value={"user_currencies": ["USD"], "user_stocks": ["AAPL"]}), \
         patch("src.views.get_currency_rates", return_value=[{"currency": "USD", "rate": 73.21}]), \
         patch("src.views.get_stock_prices", return_value=[{"stock": "AAPL", "price": 150.12}]):
